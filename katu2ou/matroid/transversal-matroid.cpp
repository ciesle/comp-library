/*
  <横断マトロイド>
    - 二部グラフG=(U,V,E)について定義される
        - 台集合はUで，「その集合を被覆するマッチングが存在する」ことを独立性として定める

    [実装/関数]
        - GraphMatroid(int V, vector<pair<int,int>> edges) : 
            グラフの頂点数(|U|+|V|)と辺集合を与えて初期化
        - int size() : Uの頂点数を返す
        - void set(vector<bool> I) : 独立集合Iに対して対応するマッチングを構成
        - vector<int> circuit(int e) : 
    
    [計算時間]
        - 独立集合Iの設定が O((n+m)*\sqrt{n})
        - I+eに対するサーキットの取得がクエリあたり O(n+m)
    
    [備考]
        - Bipartite matching / Dulmage-Mendelsohn decomposition / SCC を用いる 
            (このコードに記載 無駄な部分も多いので削ぎ落とせるはず)
