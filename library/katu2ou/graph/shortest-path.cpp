/*
  <単一始点最短路>
    ...
    [実装/関数]
        - shortest_path<ll , INF> graph(N) : 辺重みの型とinfの値を指定して作る
        - void graph.add_edge(int a, int b, ll c) : 頂点aからbへの重みcの辺を追加
        - void graph.solve(int s,int t) : sからの単一始点最短路問題を解く
            - 負の重みの辺が存在するならBellman-Ford法
            - 全ての辺重みが非負で，非零の辺重みの値が高々1通りなら，0-1BFSで解く
            - 全ての辺重みが非負で，辺重みの最大値が十分小さければDial法で解く
            - それ以外ならdijkstra法で解く
        - vector<int> retrieve_path(int t) : tまでの最短路を返す(s,...,t)

        - graph.dist[i] : solveの後に，頂点iまでの最短距離を持つ
    ...
    [参照]
        - https://hitonanode.github.io/cplib-cpp/graph/shortest_path.hpp
