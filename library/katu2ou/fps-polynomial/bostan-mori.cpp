//formal-power-series.cppが必要

/*
- mint LinearRecurrence(k,Q,P) : [x^k]P(x)/Q(x) を求める
- kitamasa(N,Q,a) : k以上の各nに対して 
                        Q_0 a_n + Q_1 a_{n-1} + ... + Q_{k} a_{n-k} = 0, Q_0 = 1
                    であるa,Qに対して a_N を求める (Qがk+1個の係数，aが最初のk項)
    - P,Q,aはFormalPowerSeries<mint> の元 
    - O(klogklogn)で解ける
*/
