/*
  <グラフマトロイド>
    ...
    [実装/関数]
        - GraphMatroid(int V, vector<pair<int,int>> edges) : 
            グラフの頂点数と辺集合を与えて初期化
        - int size() : 
        - void set(vector<bool>) : 
...
class GraphMatroid {
    public:
        int V;                              // グラフの頂点数
        vector<pair<int,int>> edges;        // 地集合：各辺を (u, v) として保持
        // 現在の独立集合 I に含まれる辺だけで構成する森林の隣接リスト
        // 各頂点に対して、{隣接頂点, その辺のインデックス} を保持する
        vector<vector<pair<int,int>>> g;
    
        // コンストラクタ：頂点数と辺のリストを渡す
        GraphMatroid(int V, const vector<pair<int,int>>& edges)
            : V(V), edges(edges) {}
        ...
        // 現在の独立集合 I に基づいて内部状態（森林）を構築する
        // I[i] == true ならば、edges[i] が独立集合に含まれているとする
        ...
        // 辺 e を現在の独立集合 I に加えた場合にできるサイクル（循環）を求める
        // サイクルがなければ空のベクトルを返す
        vector<int> circuit(int e) {
            int u = edges[e].first, v = edges[e].second;
            // 辺 e の両端が既に連結していなければサイクルは発生しない
            vector<bool> visited(V, false);
            vector<int> parent(V, -1);     // 各頂点の親
            vector<int> parentEdge(V, -1);   // 親との間に使われた辺のインデックス
            queue<int> que;
            visited[u] = true;
            que.push(u);
            bool found = false;
            while (!que.empty() && !found) {
                ...
                que.pop();
                for (auto &p : g[cur]) {
                    int nxt = p.first, edgeIndex = p.second;
                    if (!visited[nxt]) {
                        visited[nxt] = true;
                        parent[nxt] = cur;
                        parentEdge[nxt] = edgeIndex;
                        if (nxt == v) { // v に到達したら終了
                            found = true;
                            break;
                        }
            ...
            if (!found) {
                // u と v が連結していなければ、e を加えてもサイクルはできない
                return vector<int>();
            }
            // u から v への一意な経路を復元する
            vector<int> cycle;
